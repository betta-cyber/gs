...
import shutil
import win32wnet
from core import fastlogger, cfg_lib
...
class WindowsMachine:
    ...
    def run_remote(self, cmd, async=False, minimized=True, output=False):
        ...
        output_data = None
        pwd = os.getcwd()
        bat_local_path = os.path.join(pwd, 'tmp\\output.bat')
        script_local_path = cfg_lib.WINDOWS_DIR
        bat_remote_path = os.path.join(self.remote_path, 'output.bat')
        script_remote_path = os.path.join(self.remote_path, cfg_lib.SCRIPT_NAME)
        output_remote_path = os.path.join(self.remote_path, 'output.xml')
        output_local_path = os.path.join(pwd, 'xml\\output.xml')
        create_file(bat_local_path, cmd)
        self.net_copy(bat_local_path, self.remote_path)
        self.net_copy(script_local_path, self.remote_path)
        ...
        # if no async and output, copy back it
        ...
        self.net_delete(script_remote_path)
        output_data = "success"
